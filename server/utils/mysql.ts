...
function createQueryEqualString(query: DataObj = {}) {
  const queryField = Object.entries(query).filter(([k, v]) =>
    typeof v === "number" ? true : v
  );
...
export class MySql {
  ...
  static selectFieldEqual(tbName: string, fields: Array<string>, query: DataObj = {}) {
    const queryStr = createQueryEqualString(query);
    return queryStr
      ? `SELECT ${fields.length ? fields.join(", ") : "*"} FROM ${tbName} WHERE ${queryStr};`
      : this.selectFieldAll(tbName, fields);
  }
  ...
  static selectFieldPage(
    ...
    } FROM ${tbName} LIMIT ${size} OFFSET ${offset};`;
  }

  static countEqualAll(tbName: string, query: DataObj = {}) {
    const queryStr = createQueryEqualString(query);
    return queryStr
      ? `SELECT COUNT(*) FROM ${tbName} WHERE ${queryStr};`
      : this.countAll(tbName);
  }

  static selectEqualPage(
    tbName: string,
    size: number = 10,
    current: number = 1,
    query: DataObj = {},
    ...
    const queryStr = createQueryEqualString(query);
    return queryStr
      ? `SELECT * FROM ${tbName} WHERE ${queryStr} ORDER BY ${order} ${orderType} LIMIT ${size} OFFSET ${offset};`
      : this.selectPage(tbName, size, current, order, orderType);
  }

  static selectFieldEqualPage(
    tbName: string,
    fields: Array<string>,
    size: number = 10,
    current: number = 1,
    ...
    return queryStr
      ? `SELECT ${
          fields.length ? fields.join(", ") : "*"
        } FROM ${tbName} WHERE ${queryStr} LIMIT ${size} OFFSET ${offset};`
      : this.selectFieldPage(tbName, fields, size, current);
  }
  ...
  static updateOne(tbName: string, id: number, data: DataObj = {}) {
    const updateStr = Object.keys(data)
      .map((k) => `${k} = ${escape(data[k])}`)
      .join(", ");
